...
export class Grid {
  ...
  initEvents() {
    ...
    for (const [position, gridItem] of this.gridItemArr.entries()) {
      ...
      gridItem.DOM.img.outer.addEventListener('click', () => {
        if (!this.isGridView || this.isAnimating || document.documentElement.classList.contains(
      ...
      gridItem.DOM.img.outer.addEventListener('mouseenter', () => {
        ...
        }).addLabel('start', 0).set([gridItem.DOM.img.outer, gridItem.DOM.img.inner], {
      ...
      gridItem.DOM.img.outer.addEventListener('mouseleave', () => {
        ...
        gsap.timeline({
          ...
          willChange: 'transform'
        }, 'start').to([gridItem.DOM.img.outer, gridItem.DOM.img.inner], {
          scale: 1
        }, 0);
      });
    }
  ...
  showContent(gridItem) {
    ...
    this.viewportGridItems = this.gridItemArr.filter(el => el != gridItem && el.DOM.el.classList.contains(
      'in-view'));
    // Remaining (not in the viewport)
    this.remainingGridItems = this.gridItemArr.filter(el => !this.viewportGridItems.includes(el) && el !=
      gridItem).map(gridItem => gridItem.DOM.el);
    ...
    this.timeline = gsap.timeline({
    ...
    }).addLabel('start', 0).set([gridItem.DOM.el, gridItem.DOM.el.parentNode.parentNode], {
      zIndex: 100
    }, 'start').set([gridItem.DOM.img.outer, gridItem.DOM.img.inner, this.viewportGridItemsImgOuter], {
      ...
      x: imageTransform.x,
      y: imageTransform.y,
      onComplete: () => gsap.set(gridItem.DOM.img.outer, {
        willChange: ''
      })
    }, 'start').to(gridItem.DOM.img.inner, {
      scale: 1,
      onComplete: () => gsap.set(gridItem.DOM.img.inner, {
        willChange: ''
      })
    }, 'start').add(() => {
    ...
    for (const [position, viewportGridItem] of this.viewportGridItems.entries()) {
      const imgOuter = viewportGridItem.DOM.img.outer;
      this.timeline.to([viewportGridItem.DOM.caption, gridItem.DOM.caption], {
        ease: 'expo',
        opacity: 0,
        delay: 0.03 * position
        ...
        x: this.getFinalTranslationValue(imgOuter, position).x,
        y: this.getFinalTranslationValue(imgOuter, position).y,
        onComplete: () => gsap.set(imgOuter, {
          willChange: ''
        }),
        delay: 0.03 * position
      }, 'start');
    }
    this.timeline.addLabel('showContent', 'start+=0.2').to([...this.DOM.contentNavItems].slice(this
      .viewportGridItems.length + 1), {
      y: '0%',
      opacity: 1,
      delay: pos => 0.03 * pos
    }, 'showContent').add(() => {
    ...
    }, 'showContent').to([this.DOM.backCtrl, this.DOM.contentNav, gridItem.contentItem.DOM.text], {
      opacity: 1
  ...
  closeContent() {
    ...
    }).addLabel('start', 0).to([this.DOM.backCtrl, this.DOM.contentNav, gridItem.contentItem.DOM.text], {
      opacity: 0
    }, 'start').to(gridItem.contentItem.DOM.title, {
      opacity: 0,
      y: '-100%',
      scaleY: 3
    }, 'start').to([...this.DOM.contentNavItems].slice(this.viewportGridItems.length + 1), {
      y: `${gsap.utils.random(100, 300)}%`,
      opacity: 0,
      delay: pos => -0.03 * pos,
    ...
    }, 'start').addLabel('showGrid', 'start+=0.2').to([this.viewportGridItems.map(gridItem => gridItem.DOM
      .caption), gridItem.DOM.caption], {
      ease: 'power4.in',
      opacity: 1
    }, 'showGrid')
  }
